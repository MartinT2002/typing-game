...
var maxEnemies = 10; // Example maximum number of enemies on screen
function dynamic_difficulty() {
    if (started && delta_time > 0 && delta_time % 5 === 0 && spawnrate < maxEnemies) {
        spawnrate++;
    }
    if (enemies.length < spawnrate) {
        enemies.push(new Enemy('', -50, 200, true, ''));
    }
}
